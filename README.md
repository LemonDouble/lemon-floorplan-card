...
대시보드 → **카드 편집**을 열면 편집기 안에 평면도가 뜬다. 거기서 끌어 옮기고
HA 저장 버튼을 누르면 끝이다.
...
옮긴 결과는 카드 설정의 `layout` 에 쌓인다. **옮긴 것만** 기록되고, 나머지는
SVG 에 구워진 기본값을 쓴다.

```yaml
layout:
  o-sofa-1: { x: 80.5, y: 237.4, w: 132.7, h: 61.7, r: -90 }
```

키는 SVG `<image>` 의 `id` 다 (`o-<에셋이름>-<번호>`). 순서가 아니라 id 로 잡으므로
나중에 오브젝트를 지우거나 추가해도 저장된 배치가 밀리지 않는다.
일부 값만 적어도 되고, 안 적은 값은 SVG 기본값이 유지된다.

> 카드가 대시보드 설정을 직접 쓰지 않는다. `getConfigElement` 로 HA 카드 편집기에
> 얹혀서 `config-changed` 만 쏘고, 저장은 HA 가 한다. `lovelace/config/save` 는
> 대시보드 전체를 읽어 다시 쓰는 것이라 버그가 나면 다른 카드까지 날린다.

`editor.html` 은 여전히 쓸 수 있다. SVG 기본 배치 자체를 바꾸고 싶을 때
(= 모든 대시보드에 반영하고 싶을 때) 그쪽을 쓴다.
