...
def main() -> None:
    ...
    # href 와 함께 data-asset-* 도 본다. 지금 이 속성을 쓰는 곳은 없지만(커튼은
    # 그림 교체 대신 좌우 두 폭으로 바뀌었다), 상태에 따라 그림을 갈아끼우는
    # 가구를 넣을 때 빠뜨리기 딱 좋은 자리라 스캔만 남겨둔다. 맵에 안 담기면
    # 런타임에 상대경로로 남고, HA 가 모르는 경로에 SPA 폴백으로 index.html 을
    # 200 으로 내주는 바람에 404 도 없이 그냥 엑박이 된다.
